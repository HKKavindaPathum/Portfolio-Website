...
export default function ProjectDetailsModal({ project, onClose }: ProjectDetailsModalProps) {
  ...
  useEffect(() => {
    if (project) {
      ...
      const visTimer = setTimeout(() => setIsVisible(false), 0);
      const renderTimer = setTimeout(() => setIsRendered(false), 300); // Wait for transition out
      document.body.style.overflow = '';
      return () => {
    ...
    >
      ...
      >
        ...
        <div className="flex-grow overflow-y-auto p-6 sm:p-8 space-y-6">
          ...
          {/* Image Showcase */}
          <div className="h-56 sm:h-80 rounded-2xl bg-gradient-to-br from-zinc-100 to-zinc-200 dark:from-zinc-800 dark:to-zinc-950 border border-zinc-200 dark:border-zinc-800 flex items-center justify-center overflow-hidden relative shadow-md">
            {project.imageUrl ? (
              <Image
          ...
          <div className="space-y-2">
            ...
            <div className="text-sm sm:text-base text-zinc-600 dark:text-zinc-400 leading-relaxed whitespace-pre-wrap font-sans">
              {project.description}
            </div>
          </div>
